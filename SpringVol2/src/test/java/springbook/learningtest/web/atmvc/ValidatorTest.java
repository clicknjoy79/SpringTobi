...
public class ValidatorTest extends AbstractDispatcherServletTest {
    ...
    public void atrvalid() throws ServletException, IOException {
        ...
        initRequest("/add.do").addParameter("id", "1")
                .addParameter("age", "-1");
        runService();
        // 컨트롤러에서 BindingResult로 받으면 컨트롤러에 모든 에러(타입변환, 검증) 처리를 위임한다.
        // 컨트롤러에서 처리하지 않으면 정상이라고 생각하고 동작한다.
        // 에러(타입, 검증)가 발생했는데 컨트롤러에서 BindingResult로 받지 않으면 컨트롤러를 실행하지 않고 Response에 400에러(Bad Request)를 전달한다.
        assertThat(((User)getModelAndView().getModel().get("user")).getId(), is(1));
        assertThat(((User)getModelAndView().getModel().get("user")).getName(), is(nullValue()));
        assertThat(((User)getModelAndView().getModel().get("user")).getAge(), is(-1));
