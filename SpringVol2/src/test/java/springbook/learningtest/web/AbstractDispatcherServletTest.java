...
public class AbstractDispatcherServletTest implements AfterRunService {
    ...
    public AbstractDispatcherServletTest addParameter(String name, String value) {
        if(this.request == null) {
            throw new IllegalStateException("request가 존재하지 않습니다.");
        }
    ...
    public AbstractDispatcherServletTest buildDispatcherServlet() throws ServletException {
        if(this.classes == null && this.locations == null &&
                this.relativeLocations == null)
            throw new IllegalStateException("리소스가 존재하지 않습니다.");
    ...
    public ConfigurableWebApplicationContext getContext() {
        if(this.dispatcherServlet == null)
            throw new IllegalStateException("DispatcherServlet이 존재하지 않습니다.");
    ...
    public AfterRunService assertViewName(String viewname) {
        assertThat(this.dispatcherServlet.getModelAndView().getViewName(), is(viewname));
        return this;
    }

    @Override
    public AfterRunService assertModel(String name, Object value) {
        assertThat(this.dispatcherServlet.getModelAndView().getModel().get(name), is(value));
        return this;
    }
